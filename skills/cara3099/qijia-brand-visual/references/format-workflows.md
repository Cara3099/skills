...
- 品牌视觉系统、品牌手册和专题展示页以 `assets/templates/html/qijia-brand-visual-system-0.3.html` 为基准：保留顶部品牌栏、横向分区导航、纸白小方格底板、编辑式阅读区和右侧深蓝线形品牌图；不要重新引入突兀的左侧菜单栏或大面积渐变。
- 文字优先保证阅读：区块标题、说明、卡片正文、表格和组件示例均使用清晰字号；不为了保留原始截图比例而缩小文字。
- 生成：`python scripts/generate_html.py data.json output.html <skill-root>`。
- 验证：检查响应式布局、链接、打印分页、内容一致性、Logo 可辨识度和桌面端横向溢出。
