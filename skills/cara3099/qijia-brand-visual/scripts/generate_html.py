...
<div class="shell"><aside class="side"><div class="brand"><img src="{logo}" alt="企加Logo"><div><b>{esc(meta["brand"])}</b><small>BRAND DOCUMENT SYSTEM 0.3</small></div></div><nav><a href="#overview"><span>00</span>今日总览</a>{nav}<a href="#actions"><span>05</span>行动清单</a></nav><div class="side-foot">{esc(meta["issue"])}<br>{esc(meta["date"])}<br>正式发布与执行以主管机关口径为准</div></aside>
