...
- 不生成新的品牌插画、猫形轮廓、猫型虚影、吉祥物变体、背景图片、装饰图形或抽象纹理。
- 只使用 skill 内提供的原始 Logo：`assets/代理猫图.png`；不得让模型重绘、描摹、矢量化、仿制或凭文字重新生成 Logo。
- 没有可用原始素材时，使用纯色、规则线、边框、留白和排版完成视觉表达，不用模型生图补位。
- 用户明确需要新增图片时，先询问用户是否提供已批准的素材；未经确认不得自行生成或替换品牌图片。
...
- 使用单据、印章、证据时间线和原始 Logo 作为品牌资产，不要做成后台首页。
- 时间线中的文字必须在 Grid/Flex 自己的栏位或行位中，绝对定位只用于纸张边角和纹理等装饰层。
- 检查 1440px 和 390px 下的换行、溢出、按钮可见性和信息遮挡。
...
5. 确认没有新增猫形虚影、品牌插画、背景图片或模型自行生成的装饰图形。
6. 检查`服务单`、`确认快照`、`证据链`等产品术语，以及客户侧没有合同、开票和确认 PDF 下载/分享入口。
7. 按媒介渲染并检查 Word、Excel、PowerPoint 和 HTML，再发布最终文件。
